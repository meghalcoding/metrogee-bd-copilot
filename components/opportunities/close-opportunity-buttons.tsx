...
export function CloseOpportunityButtons({ opportunityId, status }: { opportunityId: string; status: "OPEN" | "ON_HOLD" | "WON" | "LOST" }) {
  ...
  return <div className="space-y-3">
    ...
    {showLost ? <div className="rounded-lg border border-border bg-background p-4"><p className="text-sm font-semibold">Why was the opportunity lost?</p><p className="mt-1 text-xs text-text-secondary">This becomes part of the deal record.</p><input value={lostReason} onChange={(e) => setLostReason(e.target.value)} className="mt-3 h-10 w-full rounded-md border border-border bg-surface px-3 text-sm" placeholder="Budget, timing, competitor..." /><div className="mt-3 flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => setShowLost(false)}>Cancel</Button><Button type="button" variant="danger" disabled={pending} onClick={markLost}>Confirm lost</Button></div></div> : null}
