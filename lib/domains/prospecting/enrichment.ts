...
function searchProvider(provider: ProspectingProvider, input: ProspectSearchInput, categoryName: string, lat: number, lon: number) {
  const radiusKm = input.radiusKm ?? 5;
  switch (provider) {
    case "osm_overpass": return searchOverpass(input.categorySlug, categoryName, lat, lon, radiusKm);
    case "geoapify": return searchGeoapify(input.categorySlug, categoryName, lat, lon, radiusKm);
    case "foursquare": return searchFoursquare(input.categorySlug, categoryName, lat, lon, radiusKm);
    case "mappls": return searchMappls(input.categorySlug, categoryName, lat, lon, radiusKm);
  }
}

function scoreMatch(business: { name: string; phone: string | null; website_url: string | null; address_line_1: string | null }, result: ProspectResult) {
  let score = 0;
  if (normalizeName(business.name) === normalizeName(result.name)) score += 60;
  else if (normalizeName(business.name).includes(normalizeName(result.name)) || normalizeName(result.name).includes(normalizeName(business.name))) score += 35;
...
function chooseMatch(business: { name: string; phone: string | null; website_url: string | null; address_line_1: string | null }, results: ProspectResult[]) {
  let best: ProspectResult | null = null;
  let bestScore = 0;
  for (const result of results) {
    const score = scoreMatch(business, result);
...
export async function enrichBusiness(
  ...
  const { data: business, error: businessError } = await supabase
    .from("businesses")
    .select("id,name,address_line_1,city,state,postal_code,country,latitude,longitude,phone,email,website_url,website_status,rating,review_count,primary_category_id,source_primary,metadata_json")
  ...
  if (latitude == null || longitude == null) {
    if (!business.postal_code) throw new Error("This business has no coordinates or Indian PIN code to use for enrichment.");
    const location = await resolveIndianPin(business.postal_code);
  ...
  if (Object.keys(update).length) {
    ...
    const { error } = await supabase.from("businesses").update(update).eq("organization_id", organizationId).eq("id", businessId);
  ...
  const enrichment = {
    ...
    providers: matched.map(({ provider, result, matchScore }) => ({ provider, provider_place_id: result.providerPlaceId, provider_url: result.providerUrl, match_score: matchScore, raw: result.raw })),
    provider_errors: providerErrors,
    field_provenance: provenance,
    conflicts,
  };
  await supabase.from("businesses").update({ metadata_json: { ...metadata, enrichment } }).eq("organization_id", organizationId).eq("id", businessId);

  for (const { provider, result } of matched) {
    const { data: source } = await supabase.from("business_sources").select("id").eq("organization_id", organizationId).eq("provider", provider).eq("provider_place_id", result.providerPlaceId).maybeSingle();
    if (source?.id) {
      await supabase.from("business_sources").update({ business_id: businessId, provider_url: result.providerUrl, raw_reference: { provider, provider_place_id: result.providerPlaceId, raw: result.raw }, last_seen_at: new Date().toISOString() }).eq("organization_id", organizationId).eq("id", source.id);
    } else {
      await supabase.from("business_sources").insert({ organization_id: organizationId, business_id: businessId, provider, provider_place_id: result.providerPlaceId, provider_url: result.providerUrl, raw_reference: { provider, provider_place_id: result.providerPlaceId, raw: result.raw } });
    }
  }

  if (matched.length || Object.keys(providerErrors).length) {
    const lines = [
      `Enrichment run: ${new Date().toLocaleString("en-IN")}`,
      matched.length ? `Matched providers: ${matched.map((item) => item.provider).join(", ")}.` : "No provider produced a confident match.",
      fieldsUpdated.length ? `Filled: ${fieldsUpdated.join(", ")}.` : "No previously empty CRM fields were filled.",
      ...Object.entries(providerErrors).map(([provider, message]) => `${provider}: ${message}`),
    ];
    await supabase.from("notes").insert({ organization_id: organizationId, business_id: businessId, author_user_id: ownerUserId, body: lines.join("\n") });
  }
  ...
  return {
    ...
    matchedProviders: matched.map((item) => item.provider),
    providerErrors,
    fieldsUpdated,
  };
}
