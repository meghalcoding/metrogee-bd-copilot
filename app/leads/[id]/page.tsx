      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-xl border border-border bg-surface">
            ...
            {tasks.length ? tasks.slice(0, 8).map((task) => <Link key={task.id} href={`/tasks/${task.id}/edit`} className="flex items-center justify-between gap-4 border-b border-border px-5 py-4 last:border-0 hover:bg-surface-muted/60"><div className="min-w-0"><p className="truncate text-sm font-medium">{task.title}</p><p className="mt-1 text-xs text-text-muted">{task.status} · {task.priority}</p></div><span className="shrink-0 text-xs text-text-muted">{task.due_at ? new Date(task.due_at).toLocaleString() : "No due date"}</span></Link>) : <div className="px-5 py-10 text-center text-sm text-text-secondary">No tasks for this lead.</div>}
          </section>
        ...
        <section className="rounded-xl border border-border bg-surface">
          ...
          {opportunities.length ? opportunities.map((opp) => <Link key={opp.id} href={`/opportunities/${opp.id}`} className="flex items-center justify-between gap-4 border-b border-border px-5 py-4 last:border-0 hover:bg-surface-muted/60"><div className="min-w-0"><p className="truncate text-sm font-medium">{opp.name}</p><p className="mt-1 text-xs text-text-muted">{opp.stage} · {opp.status}</p></div><p className="shrink-0 text-sm font-semibold">{opp.value_amount != null ? `${opp.currency} ${Number(opp.value_amount).toLocaleString()}` : "—"}</p></Link>) : <div className="px-5 py-10 text-center text-sm text-text-secondary">No opportunities yet.</div>}
        </section>
      </div>
    </AppShell>
  );
