...
export default async function OpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><div className="space-y-6">
    ...
    <section className="flex flex-col justify-between gap-4 rounded-xl border border-border bg-surface p-5 sm:flex-row sm:items-start sm:p-6">
      ...
      <div className="flex flex-wrap gap-2"><Button asChild variant="secondary"><Link href={`/opportunities/${id}/edit`}><Pencil className="size-4"/>Edit</Link></Button>{<CloseOpportunityButtons opportunityId={id} status={opportunity.status}/>}</div>
    </section>
